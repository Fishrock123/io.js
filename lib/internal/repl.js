...
function setupHistory(repl, historyPath, oldHistoryPath, ready) {
  if (!historyPath) {
    try {
      ...
      repl._writeToOutput('Error: Could not get the home directory.\n' +
                          'REPL session history will not be persisted.\n');
      util.debuglog(err.stack);
      ready(null, repl);
    }
  }
  ...
  function onread(err, data) {
    ...
    if (data) {
      ...
      repl._writeToOutput('Converting old repl history to new repl history.\n');
      try {
...
function _replHistoryMessage() {
  if (this.history.length === 0) {
    this._writeToOutput(
        ...
        'Set the NODE_REPL_HISTORY environment variable to ' +
