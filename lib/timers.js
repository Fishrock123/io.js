...
const Heap = require('internal/heap');
...
function unrefTimeout() {
  ...
  while (first = unrefHeap.root) {
    ...
    unrefHeap.remove(first, compareTimer);
...
exports._unrefActive = function(item) {
  ...
  if (unrefHeap.root === item || item._heapParent || item._heapLeft || item._heapRight) {
    unrefHeap.remove(item, compareTimer);
  }
  ...
  if (unrefHeap.root === null) {
    debug('unrefHeap empty');
  ...
  unrefHeap.insert(item, compareTimer);
...
const _uA_enroll = exports._uA_enroll = function(item, msecs) {
  if (typeof msecs !== 'number') {
    throw new TypeError('msecs must be a number');
  }
...
const _uA_unenroll = exports._uA_unenroll = function(item) {
  // if empty then stop the watcher
  debug('_uA_unenroll');
...
function compareTimer(a, b) {
  const timeA = a._idleStart + a._idleTimeout;
  const timeB = b._idleStart + b._idleTimeout;
